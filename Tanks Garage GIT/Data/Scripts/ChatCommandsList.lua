...
commands = {
    ...
    ["/xp"] = {
        OnCommandCalledClient = function(player, message)
            ...
            Chat.LocalMessage(
                "ERROR: command does not contain a tankID or valid resource amount. Format: /xp <tankID> <RPvalue>"
            )
        end,
        OnCommandCalledServer = function(player, message)
            local resourceFound = nil
    ...
    ["/freexp"] = {
        OnCommandCalledClient = function(player, message)
            Chat.LocalMessage("Setting Free XP resource to " .. tostring(100000))
        end,
        OnCommandCalledServer = function(player, message)
            player:SetResource("Free XP", 100000)
        end,
        OnCommandReceivedClient = function(player, message)
        end,
        description = "Set the XP resource of all tanks to 100k. Format: /xp <tankID> <RPvalue>",
        requireMessage = false,
        adminOnly = true,
        adminRank = AdminData.AdminRanks.HigherAdmin
    },
