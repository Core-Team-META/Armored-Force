...
function API.GetTimeUntilEvent(eventId)
    local gameEvent = CorePlatform.GetGameEvent(eventId)
    local currentTime = DateTime.CurrentTime()
    if gameEvent.state == CoreGameEventState.ACTIVE then
        local endTime = gameEvent:GetEndDateTime()
        return endTime.secondsSinceEpoch - currentTime.secondsSinceEpoch
    elseif gameEvent.state == CoreGameEventState.SCHEDULED then
        local startTime = gameEvent:GetStartDateTime()
        return startTime.secondsSinceEpoch - currentTime.secondsSinceEpoch
    end
