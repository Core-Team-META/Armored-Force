...
function AddLine(line, color)
	...
	lines[1].weaponUsed = line[3] or "TankDeath"
...
function GetTeamColor(player)
	if not Object.IsValid(player) then return FRIENDLY_COLOR end
...
function OnKill(killerPlayer, killedPlayer, sourceObjectId, extraCode)
	...
	--[[
		...
		feedTable[3] = "TankDeath" --sourceObject or nil -- weaponUsed (weapon)
...
function OnDamaged(killerPlayer, killedPlayer, sourceObjectId, extraCode)
	local lineColor = TEXT_COLOR
	local killerColor = TEXT_COLOR
	local killedColor = TEXT_COLOR
